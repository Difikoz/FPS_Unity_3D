        ...
        protected override void OnSpawn()
        {
            if (_spawnedControllers.Count > 0)
            {
                for (int i = _spawnedControllers.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        GameManager.StaticInstance.ControllersManager.RemoveController(_spawnedControllers[i]);
                        //_spawnedControllers[i].ResetComponents();
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                GameManager.StaticInstance.ControllersManager.AddController(npc);
                _spawnedControllers.Add(npc);
                npc.Initialize(_faction);
            }
        }
    }
}